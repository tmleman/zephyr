...
static int intel_adsp_ipc_send(const struct device *dev, void *token, const void *data, size_t len)
{
	...
	const uint32_t * msg = (uint32_t *)data;
 ...
 *
 * This backend uses the TX buffer size query as a way to check whether
 * the host is ready to receive the next message. When the IPC channel
 * is idle (no BUSY bit set and no pending TX acknowledgment), a single
 * “buffer” of two 32-bit words is considered available and the function
 * returns sizeof(uint32_t) * 2. When the channel is still busy, no
 * buffer is available and the function returns 0.
 *
 * Return values:
...
/*
 * This backend does not need to explicitly hold RX buffers because the
 * IPC channel is effectively held from the moment the message is
 * received in the interrupt handler until the firmware completes the
 * handling. However, we must still provide a hold_rx_buffer()
 * implementation so that ipc_service_release_rx_buffer() can check
 * both hold and release callbacks and allow the use of
 * ipc_service_release_rx_buffer() to notify the host that the channel
 * is available again.
 */
...
static int intel_adsp_ipc_send_critical(const struct device *dev, void *token, const void *data, size_t len)
{
	ARG_UNUSED(token);
	int ret;
